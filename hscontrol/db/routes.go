...
/*func SaveNodeRoutes(tx *gorm.DB, node *types.Node) (bool, error) {
	...
	// Collect routes for batch operations
	var updateRouteIDs []uint64
	var deleteRouteIDs []uint64
	var newRoutes []types.Route

	// Process existing routes
	// get update Routes and delete Routes
	for _, route := range currentRoutes {
		prefix := netip.Prefix(route.Prefix)
		if _, isAdvertised := advertisedRoutes[prefix]; isAdvertised {
			if !route.Advertised || !route.Enabled { //does !route.Enabled need?
				// Need to update: make it advertised and enabled
				updateRouteIDs = append(updateRouteIDs, uint64(route.ID))
				sendUpdate = true
			}
			...
			//	Route is no longer advertised - mark for deletion
			if route.Advertised || route.Enabled {
				deleteRouteIDs = append(deleteRouteIDs, uint64(route.ID))
				sendUpdate = true
			}
	...
	// Batch update routes to be advertised and enabled
	if len(updateRouteIDs) > 0 {
		err := tx.Model(&types.Route{}).
			Where("id IN ?", updateRouteIDs).
			Updates(types.Route{
		...
		log.Info().
			Uints64("routeIDs", updateRouteIDs).
			Msg("Batch updated routes to advertised and enabled")
	}

	// Batch delete unused routes
	if len(deleteRouteIDs) > 0 {
		err := tx.Unscoped().Delete(&types.Route{}, deleteRouteIDs).Error
		if err != nil {
			return sendUpdate, err
	...
	// Create new routes (auto-enabled)
	...
	if len(newRoutes) > 0 {
		...
		log.Debug().
			Int("count", len(newRoutes)).
			Msg("Batch created new auto-enabled routes")
		sendUpdate = true
	}
...
/*
func processNodeRoutes(tx *gorm.DB, node *types.Node) error {
	dbRoutes := types.Routes{}
	newRoutes := types.Routes{}
	var updateRoutes []uint
	var delRoutes []uint

	tx := db.Begin()

	err := tx.Where("node_id = ?", node.ID).Find(&dbRoutes).Error
	if err != nil {
		tx.Rollback()
		return err
	}
	...
	advertisedRoutes := map[netip.Prefix]bool{}
	for _, p := range node.HostInfo.RoutableIPs {
		advertisedRoutes[p] = false
	}

	// get update Routes and delete Routes
	for _, route := range dbRoutes {
		pref := netip.Prefix(route.Prefix)
		if _, ok := advertisedRoutes[pref]; ok {
			if !route.Enabled || !route.Advertised || route.IsPrimary {
				updateRoutes = append(updateRoutes, route.ID)
			}
			advertisedRoutes[pref] = true
		} else {
			delRoutes = append(delRoutes, route.ID)
		}
	}
	// Batch update routes
	if len(updateRoutes) > 0 {
		err := tx.Model(types.Route{}).Where("id IN ?", updateRoutes).
			Updates(types.Route{
				Enabled:    true,
				Advertised: true,
				IsPrimary:  false}).Error
		...
		if err != nil {
			log.Error().
				Msgf("Update Routes Error: %v", err)
			tx.Rollback()
			return err
		}
	}
	...
	if len(delRoutes) > 0 {
		...
		if err != nil {
			log.Error().
				Msgf("Remove Routes Error: %v", err)
			tx.Rollback()
			return err
		}
	}
	...
	if len(newRoutes) > 0 {
		...
		if err != nil {
			log.Error().
				Msgf("Create Routes Error: %v", err)
			tx.Rollback()
			return err
		}
	}
	...
	if len(updateRoutes) > 0 || len(delRoutes) > 0 || len(delRoutes) > 0 {
		h.updateRoutesCache()
	}
